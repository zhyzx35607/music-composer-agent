...
| **反馈修改（回炉）** | 读取父版本 music_json，AI 只输出结构化 revision_plan，Python 定向应用并生成新版本 |
...
后端 MusicService.revise()
  ├─ 定位 outputs/{parent_version_id}.json 作为不可变父谱
  ├─ runPipeline(..., "revise", --revision-source-json parent.json)
  │   → Python 将父谱和反馈发送给 GPT
  │   → GPT 只返回 revision_plan，不返回完整 music_json
  │   → Python 校验范围/轨道/修改比例并将计划应用到父谱
  │   → 新 music_json → MIDI → WAV
  │   → 输出 {version_id}.revision_plan.json 作为修改证据

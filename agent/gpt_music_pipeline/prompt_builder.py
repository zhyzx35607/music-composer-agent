...
def build_prompt(
    ...
    cleaned_request = user_request.strip() or "Generate pure instrumental music based on the selected UI parameters."

    template = TEMPLATE_PATH.read_text(encoding="utf-8")
    return (
        template.replace("{{USER_REQUEST}}", cleaned_request)
        .replace(
            "{{UI_PARAMETERS_JSON}}",
